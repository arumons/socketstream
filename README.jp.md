...
トンネルはクライアントを接続した直後に生成されます。また、切断しても自動的に再生成されます。
...
SocketStreamはリアルタイムデータ(チャット、株式取引、位置のモニタリング、分析など)を扱うモダンなアプリケーションづくりに最適です。◆→bekkou: 漢字＋「等」になる時は「等」はひらがなにすると読みやすく感じます←◆◆→bekkou: "ロケーションモニタリング"でググったら 7件 しかヒットしなかったのでよく使われていそうな表現にしました←◆
しかし現時点では、SEOのためにユニークなURLを必要とするブログ等のコンテンツリッチなサイトには不向きです。
