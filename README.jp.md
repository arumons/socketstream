...
すべての設定可能な環境変数★は近日公開予定のサイトにてお知らせします。現在は SocketStreamコンソール上で SS.config とタイプすると設定可能な環境変数★をすべて見られます。
...
使い方はシンプルです。'exports.' をプリフィックスにしていないすべてのブジェクト、関数、変数は自動的にプライベートになります。そうすることでそれらは APIツリーに追加され、同じ環境のあらゆるファイルからアクセスできるようになります。
...
**まめ知識** キーストロークをとことん少なくしたいなら、SS.client へのエイリアスをつくれます。
